...
/*   Updated: 2023/11/29 12:47:13 by mvan-pee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	map_check(char **map_split, t_game *game)
{
	...
	nb_table = 0;
	while (map_split[nb_table])
		nb_table++;
	len = ft_strlen(map_split[0]);
	count = 0;
	while (map_split[count])
	{
		if (map_line_check(map_split[count], len, nb_table, count))
			return (1);
	...
	if (game->player != 1 || game->coin <= 0 || game->exit != 1)
		return (ft_printf_fd(2, "Error\nNumber of item is incorrect. P %d C %d E \
